...
describe Rouge::Builtins do
  ...
  describe "list" do
    it "should create the empty list" do
      Rouge.eval(@context, Rouge.read("(list)")).should eq Rouge.read('()')
    end

    it "should create a unary list" do
      Rouge.eval(@context, Rouge.read('(list "trent")')).
          should eq Rouge.read('("trent")')
      Rouge.eval(@context, Rouge.read("(list true)")).
          should eq Rouge::Cons[true]
    end

    it "should create an n-ary list" do
      Rouge.eval(@context, Rouge::Cons[Rouge::Symbol[:list], *(1..50)]).
          should eq Rouge::Cons[*(1..50)]
    end
  end
    ...
    describe "argument binding" do
      it "should bind place arguments correctly" do
        Rouge.eval(@context, Rouge.read('(fn [a] a)')).call(:zzz).
            should eq :zzz
        Rouge.eval(@context, Rouge.read('(fn [a b] (list a b))')).
            call(:daria, :morgendorffer).
            should eq Rouge::Cons[:daria, :morgendorffer]
      end

      it "should bind rest arguments correctly" do
        Rouge.eval(@context, Rouge.read('(fn (y z & rest) (list y z rest))')).
